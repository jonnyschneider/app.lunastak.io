...
| Event | Side | Value | Metadata | What it means |
|---|---|---|---|---|
| `cta_upload_doc` | client | `first-time` \| `overflow-menu` \| `kb-empty-state` | `projectId`, userType | User opened the document upload dialog. **Watch `kb-empty-state`.** Upload was dropped from the empty state while narrowing it to two cards, and no document was uploaded on production between March and September 2026 as a result — the backend was live the whole time. Restored 2026-09-09 with its own surface value so the question "is anyone starting from a document?" is answerable without reading the code. A flatline here means the door has gone again. |
